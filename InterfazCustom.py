...
# Llama a java y calcula la ruta
def calculate_route():
    ...
    result = subprocess.run(command, cwd=os.getcwd()+"\src", capture_output=True, text=True, shell=True)

    #   v------------ Desactivada
    # if(len(str(result.stderr))==-10):
    #     consola_error.configure(text="ERROR: " + str(result.stderr))
    # else:
    #     consola_error.configure(text="")

    input_str = result.stdout
    number = 0
    ...
    # Cadena String con las rutas -> Array Strings con la ruta
...
#  __/ /\_,_/|___/\_,_/                  
# |___/                                  
# iniciamos compilando nuestro algoritmo en Java.
compilar = [
"javac",
 ".\src\ia\*.java"
]
...
print(str(os.getcwd()))
result = subprocess.run(compilar, cwd=os.getcwd(), capture_output=True, text=True)
print(result.stderr)
